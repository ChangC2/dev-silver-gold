d3.gantt = function (container_name, width, timeDomainMode) {
	...
	function hoverIn(ctx, val) {
		...
		d3.select('#tooltip-time').text(
			moment(val.startDate).format("h:mm A") +
			" - " +
			moment(val.endDate).format("h:mm A")
		);

		var duration = val.endDate - val.startDate;
		...
		d3.select('#tooltip-duration').text(timeString);
