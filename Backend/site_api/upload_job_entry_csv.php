...
if (isset($_FILES['file'])) {
	...
	try {
        ...
        while(!$savedfile->eof())
        {
            ...
            $where = "(`jobID`='$jobId' AND `order_type`='$orderType' AND `seq_no`='$seqNo' AND `status_type_val`='$statusTypeVal' AND `pr_order_no`='$prOrderNo' AND `customer`='$customer' AND `partNumber`='$partNumber' AND `description`='$description' AND `qtyRequired`='$qtyRequired' AND `dueDate`='$dueDate' AND `pr_center_no`='$prCenterNo' AND `short_desc`='$shortDesc')";
            $table = new TableMgr();
            $tableName = $customer_id."_jobdata";
            
            $res = $table->ReadTable($tableName, $where, "");
            if (count($res) == 0) {
                $insertQuery = "INSERT INTO " . $customer_id . "_jobdata (`jobId`, `order_type`, `seq_no`, `status_type_val`, `jobID_long`, `customer`, `partNumber`, `description`, `qtyRequired`, `dueDate`, `aux1data`, `aux2data`, `aux3data`, `pr_center_no`, `short_desc`) VALUES 
                                                                ('$jobId', '$orderType', '$seqNo', '$statusTypeVal', '$jobID_long', '$customer', '$partNumber', '$description', '$qtyRequired', '$dueDate', '$aux1data', '$aux2data', '$aux3data', '$prCenterNo', '$shortDesc')";
