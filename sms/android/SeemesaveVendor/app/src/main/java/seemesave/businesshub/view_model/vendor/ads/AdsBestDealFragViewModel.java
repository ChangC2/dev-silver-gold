...
public class AdsBestDealFragViewModel extends ViewModel {
    ...
    public AdsBestDealFragViewModel(){
    ...
    public void loadLocalData() {
        try {
            String data = DatabaseQueryClass.getInstance().getData(App.getUserID(), "AdsBestDeal", "");
            if (!TextUtils.isEmpty(data)) {
                AdsBestDealListRes localRes = GsonUtils.getInstance().fromJson(data, AdsBestDealListRes.class);
    ...
    public void loadData(String promotion_type, String keyword, String from_date, String end_date) {
        AdsApi.getAdsBestDealList(promotion_type, keyword, from_date, end_date, offset.getValue(), 20);
    }

    @Subscribe(threadMode = ThreadMode.MAIN)
    public void onSuccessData(AdsBestDealListRes res) {
        setIsBusy(false);
        if (res.isStatus()) {
            ...
            if (offset.getValue() == 0) {
                String data = new Gson().toJson(res, new TypeToken<AdsBestDealListRes>() {
                }.getType());
