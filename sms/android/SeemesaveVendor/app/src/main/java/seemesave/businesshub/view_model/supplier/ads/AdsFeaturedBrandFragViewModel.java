...
public class AdsFeaturedBrandFragViewModel extends ViewModel {
    ...
    public AdsFeaturedBrandFragViewModel(){
    ...
    public void loadLocalData() {
        try {
            String data = DatabaseQueryClass.getInstance().getData(App.getUserID(), "AdsFeaturedBrand", "supplier");
            if (!TextUtils.isEmpty(data)) {
                AdsFeaturedBrandListRes localRes = GsonUtils.getInstance().fromJson(data, AdsFeaturedBrandListRes.class);
    ...
    public void loadData(String promotion_type, String keyword, String from_date, String end_date) {
        AdsApi.getAdsFeaturedBrandList(promotion_type, keyword, from_date, end_date, offset.getValue(), 20);
    }

    @Subscribe(threadMode = ThreadMode.MAIN)
    public void onSuccessData(AdsFeaturedBrandListRes res) {
        setIsBusy(false);
        if (res.isStatus()) {
            ...
            if (offset.getValue() == 0) {
                String data = new Gson().toJson(res, new TypeToken<AdsFeaturedBrandListRes>() {
                }.getType());
