...
public class DeliverOrderDetailActivity extends BaseActivity {
    ...
    private DeliverOrderDetailActivity activity;
    ...
    private DeliverOrderModel collectOrderModel = new DeliverOrderModel();
    ...
    void apiCallForGetPromotions(boolean show_load) {
        if (G.isNetworkAvailable(activity)) {
            if (show_load)
                showLoadingDialog();
            String url = String.format(Locale.US, G.GetDeliverOrderDetail, String.valueOf(order_id));
            Ion.with(activity)
                    .load(url)
                            ...
                            if (e != null) {
                                ...
                                try {
                                    ...
                                    if (jsonObject.getBoolean("status")) {
                                        Gson gson = new Gson();
                                        collectOrderModel = gson.fromJson(jsonObject.getJSONObject("data").getString("CartInfo"), DeliverOrderModel.class);
                                        ...
                                        Glide.with(activity)
                                                ...
                                                .into(imgUser);
                                        if (collectOrderModel.getCart_status() == 2) {
                                            txtOrderStatus.setText("Ready");
                                        } else if (collectOrderModel.getCart_status() == 12) {
                                            txtOrderStatus.setText("Picking");
                                        } else if (collectOrderModel.getCart_status() == 13) {
                                            txtOrderStatus.setText("Ready to deliver");
                                        } else if (collectOrderModel.getCart_status() == 50) {
                                            txtOrderStatus.setText("Deliver Failed");
                                        } else if (collectOrderModel.getCart_status() == 30 || collectOrderModel.getCart_status() == 31) {
                                            txtOrderStatus.setText("Delivered");
                                        } else {
                                            txtOrderStatus.setText("Delivering");
                                        }
                                        txtNote.setText(collectOrderModel.getDelivery_note());
                                        txtAddress.setText(String.format("%1$s %2$s %3$s %4$s %5$s %6$s %7$s", collectOrderModel.getDelivery_street1(), collectOrderModel.getDelivery_street2(), collectOrderModel.getDelivery_suburb(), collectOrderModel.getDelivery_city(), collectOrderModel.getDelivery_state(), collectOrderModel.getDelivery_country(), collectOrderModel.getDelivery_postal_code()));
                                        txtContactNumber.setText(collectOrderModel.getDelivery_contact_number());
    ...
    private void onUpdateStock() {
        ...
        showLoadingDialog();
        Ion.with(activity)
                .load("PUT", G.SetDeliverUpdateStock)
                .addHeader("Authorization", App.getToken())
                .addHeader("Content-Language", App.getPortalToken())
                        ...
                        if (e != null) {
                            ...
                            try {
                                ...
                                if (jsonObject.getBoolean("status")) {
                                    ...
                                    LocalBroadcastManager.getInstance(activity).sendBroadcast(new Intent("refresh_deliver_data"));
                                    finish();
                                } else {
                                    Toast.makeText(activity, jsonObject.optString("message"), Toast.LENGTH_LONG).show();
                                }
