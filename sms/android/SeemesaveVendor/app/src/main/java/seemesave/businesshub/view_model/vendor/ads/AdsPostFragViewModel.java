...
public class AdsPostFragViewModel extends ViewModel {
    ...
    public void loadLocalData() {
        try {
            String data = DatabaseQueryClass.getInstance().getData(App.getUserID(), "Post", String.valueOf(App.getPortalType()));

            if (!TextUtils.isEmpty(data)) {
                AdsPostListRes localRes = GsonUtils.getInstance().fromJson(data, AdsPostListRes.class);
    ...
    public void loadData(String keyword, String from_date, String end_date) {
        AdsApi.getPostList(keyword, from_date, end_date, offset.getValue(), 20);
    }

    @Subscribe(threadMode = ThreadMode.MAIN)
    public void onSuccessData(AdsPostListRes res) {
        setIsBusy(false);
        if (res.isStatus()) {
            ...
            if (offset.getValue() == 0) {
                String data = new Gson().toJson(res, new TypeToken<AdsPostListRes>() {
                }.getType());
