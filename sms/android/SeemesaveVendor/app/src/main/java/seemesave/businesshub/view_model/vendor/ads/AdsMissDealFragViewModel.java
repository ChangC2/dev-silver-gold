...
public class AdsMissDealFragViewModel extends ViewModel {
    ...
    public AdsMissDealFragViewModel(){
    ...
    public void loadLocalData() {
        try {
            String data = DatabaseQueryClass.getInstance().getData(App.getUserID(), "AdsCantMissDeal", "");
            if (!TextUtils.isEmpty(data)) {
                AdsMissDealListRes localRes = GsonUtils.getInstance().fromJson(data, AdsMissDealListRes.class);
    ...
    public void loadData(String promotion_type, String keyword, String from_date, String end_date) {
        AdsApi.getAdsMissDealList(promotion_type, keyword, from_date, end_date, offset.getValue(), 20);
    }

    @Subscribe(threadMode = ThreadMode.MAIN)
    public void onSuccessData(AdsMissDealListRes res) {
        setIsBusy(false);
        if (res.isStatus()) {
            ...
            if (offset.getValue() == 0) {
                String data = new Gson().toJson(res, new TypeToken<AdsMissDealListRes>() {
                }.getType());
