...
public class AdsFeaturedStoreFragViewModel extends ViewModel {
    ...
    public AdsFeaturedStoreFragViewModel(){
    ...
    public void loadLocalData() {
        try {
            String data = DatabaseQueryClass.getInstance().getData(App.getUserID(), "AdsFeaturedStore", "");
            if (!TextUtils.isEmpty(data)) {
                AdsFeaturedStoreListRes localRes = GsonUtils.getInstance().fromJson(data, AdsFeaturedStoreListRes.class);
    ...
    public void loadData(String keyword, String from_date, String end_date) {
        AdsApi.getFeaturedStores(keyword, from_date, end_date, offset.getValue(), 20);
    }

    @Subscribe(threadMode = ThreadMode.MAIN)
    public void onSuccessData(AdsFeaturedStoreListRes res) {
        setIsBusy(false);
        if (res.isStatus()) {
            ...
            if (offset.getValue() == 0) {
                String data = new Gson().toJson(res, new TypeToken<AdsFeaturedStoreListRes>() {
                }.getType());
