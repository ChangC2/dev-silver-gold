...
public class AgentApi {
    public static void getAgentList(int offset, int page_size, String keyword) {
        RetrofitClient.getClient(ApiConstants.BASE_URL)
                .create(Agent.class)
                .getAgentList(App.getToken(), App.getPortalToken(), offset, page_size, keyword)
                .enqueue(new Callback<CommonRes>() {
                    @Override
                    public void onResponse(Call<CommonRes> call, Response<CommonRes> result) {
                        AgentListRes res = new AgentListRes();
                        if (result.isSuccessful()) {
                            if (result.body().isStatus()) {
                                String dataStr = GsonUtils.getInstance().toJson(result.body().getData());
                                res = GsonUtils.getInstance().fromJson(dataStr, AgentListRes.class);
                            } else {
                    ...
                    public void onFailure(Call<CommonRes> call, Throwable t) {
                        EventBus.getDefault().post(new AgentListRes());
                    }
                });
