...
public class AdsExclusiveDealFragViewModel extends ViewModel {
    ...
    public AdsExclusiveDealFragViewModel(){
    ...
    public void loadLocalData() {
        try {
            String data = DatabaseQueryClass.getInstance().getData(App.getUserID(), "AdsExclusiveDeal", "");
            if (!TextUtils.isEmpty(data)) {
                AdsExclusiveDealListRes localRes = GsonUtils.getInstance().fromJson(data, AdsExclusiveDealListRes.class);
    ...
    public void loadData(String promotion_type, String keyword, String from_date, String end_date) {
        AdsApi.getAdsExclusiveDealList(promotion_type, keyword, from_date, end_date, offset.getValue(), 20);
    }

    @Subscribe(threadMode = ThreadMode.MAIN)
    public void onSuccessData(AdsExclusiveDealListRes res) {
        setIsBusy(false);
        if (res.isStatus()) {
            ...
            if (offset.getValue() == 0) {
                String data = new Gson().toJson(res, new TypeToken<AdsExclusiveDealListRes>() {
                }.getType());
