...
if dpkg --print-foreign-architectures | grep -q '^armhf$'; then
    echo "Architecture 'armhf' is already added."
else
    echo "Architecture 'armhf' is not added. Adding it now."
    sudo dpkg --add-architecture armhf
    sudo apt-get update
fi

if dpkg -l | grep -q '^ii  qemu '; then
    echo "Package qemu is already installed."
else
    echo "Package qemu is not installed. Installing now..."
    sudo apt-get install qemu -y
fi

if dpkg -l | grep -q '^ii  qemu-user '; then
    echo "Package qemu-user is already installed."
else
    echo "Package qemu-user is not installed. Installing now..."
...
if dpkg -l | grep -q '^ii  libqt5serialport5:armhf '; then
    echo "Package libqt5serialport5:armhf is already installed."
else
    echo "Package libqt5serialport5:armhf is not installed. Installing now..."
...
if dpkg -l | grep -q '^ii  libqt5widgets5:armhf '; then
    echo "Package libqt5widgets5:armhf is already installed."
else
    echo "Package libqt5widgets5:armhf is not installed. Installing now..."
