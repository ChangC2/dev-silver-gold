...
void loadPref() {
  ...
  deviceidstr = pref.getString("DEVICEID", "");
  // customeridstr = String("visser");
  // deviceidstr = String("QPQLO2RXV6");
  ...
  device_id_str = deviceidstr.c_str();
  ip_str = ipstr.c_str();
...
void postGanttData() {
  HTTPClient http;
  String apiUrl = DATA_API_URL + urlEncode(deviceidstr) + String("&signals=");
  for (int i = 0; i < 4; i++) {
    din[i] = digitalRead(din_pin[i]);
  }
  apiUrl = apiUrl + String(din[0] ? "0" : "1");
  apiUrl = apiUrl + String(din[1] ? ",0" : ",1");
  apiUrl = apiUrl + String(din[2] ? ",0" : ",1");
  apiUrl = apiUrl + String(din[3] ? ",0" : ",1");
