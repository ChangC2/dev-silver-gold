...
public final class BottomSheetView: UIView {
    ...
    public func present(in superview: UIView, targetIndex: Int = 0, animated: Bool = true, completion: ((Bool) -> Void)? = nil) {
        ...
        if animated {
            tConstraint = topAnchor.constraint(equalTo: superview.topAnchor, constant: superview.frame.height)
        } else {
            dimView.alpha = 1.0
            tConstraint = topAnchor.constraint(equalTo: superview.topAnchor, constant: startOffset)
        }

        springAnimator.addAnimation { [weak self] position in
            guard let self = self else { return }
            self.updateDimViewAlpha(for: position.y)
            self.tConstraint.constant = position.y
    ...
    private func animate(to offset: CGFloat, with initialVelocity: CGPoint = .zero) {
        ...
        springAnimator.fromPosition = CGPoint(x: 0, y: tConstraint.constant)
    ...
    private func updateDimViewAlpha(for offset: CGFloat) {
        if offset <= tConstraint.constant && dimView.alpha == 1 {
            return
        }
    ...
    @objc private func handlePan(panGesture: UIPanGestureRecognizer) {
        initialOffset = initialOffset ?? tConstraint.constant
        ...
        case .changed:
            ...
            tConstraint.constant = translationTarget.nextOffset(for: location)
