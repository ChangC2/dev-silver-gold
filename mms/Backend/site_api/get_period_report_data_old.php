...
if ($machine_id == '' || $machine_id == '' || $machine_id == '') {
    ...
    try {
        ...
        $where = "machine_id IN ('" . implode("','", array_map('strVal', $machine_id)) . "') AND str_to_date(date,'%m/%d/%Y')>=str_to_date('$startDate','%m/%d/%Y') AND str_to_date(date,'%m/%d/%Y')<=str_to_date('$endDate','%m/%d/%Y')";
