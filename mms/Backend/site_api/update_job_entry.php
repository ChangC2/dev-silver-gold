...
if ($customer_id != '') {
    ...
    $query = "UPDATE `ics_jobdata` SET `files` = '$files', `partNumber` = '$partNumber', `programNumber` = '$programNumber', `description` = '$description', `partsPerCycle` = $partsPerCycle, `targetCycleTime` = $targetCycleTime, `qtyRequired` = $qtyRequired, `qtyCompleted` = $qtyCompleted, `aux1data` = '$aux1data', `aux2data` = '$aux2data', `aux3data` = '$aux3data' WHERE `ics_jobdata`.`Id` = $entry_id";
