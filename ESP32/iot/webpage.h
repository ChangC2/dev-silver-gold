    ...
    <body>
      <div class="container">
        ...
        <div class="sub-container">
          ...
          />
          <button id="keysetting" onclick="setKey()" class="settings-button">
            Save
          </button>
        </div>
        <div class="sub-container">
        ...
        <div class="sub-container">
          ...
          <button id="ipsetting" onclick="setWifi()" class="wifi-settings-button">
            Save
          </button>
        </div>
      </div>
...
const char SCRIPT[] PROGMEM = R"=====(
  ...
  function getKey() {
    const sensor_key = document.getElementById("sensor_key").value;
    if (sensor_key !== "") return;
    console.log("getKey request: ");
    this.request(apiGetKey, "get", {}, (response) => {
      console.log("getKey response: ", response, typeof response);
      if (response) {
        document.getElementById("sensor_key").value = response.sensor_key;
      }
    });
  }
  ...
  function setKey() {
    const sensor_key = document.getElementById("sensor_key").value;
    const data = { sensor_key };
    console.log(data);
    this.request(apiSetKey, "post", data, (response) => {});
  }
