<script>
  new Choices("#voice_status", {
    shouldSort: !1
  });
  new Choices("#user", {
    shouldSort: !1
  });
  $('#voice_status').change(function(e) {
    var selected_user = $("#user :selected").val();
    var selected_status = $("#voice_status :selected").val();
    window.location.href = '<?php echo base_url('voices?token=' . urlencode($token)); ?>' + '&selected_user=' + selected_user + '&selected_status=' + selected_status
  });
  ...
  if (permission == 0) {
    ...
    voicesTbl = $('#voices-table').DataTable({
      ...
      columnDefs: [{
        ...
        render: function(data, type, row) {
          return (data == 0) ? '<p style="color: red;font-size:12px;">Technician will call to review your voice for approval.</p>' :
            '<p style="color: green;font-size:14px;">Approved.</p>';
        }
      }, {
        targets: [3],
        orderable: false,
...
<script>
  ...
  var onClickSave = function() {
    ...
    if (addType == 0) {
      ...
      showLoading();
      $.ajax({
        url: "<?php echo base_url('voices/audioUpload'); ?>",
        type: 'POST',
        data: form,
        processData: false,
        contentType: false,
  ...
  var add = function(url) {
    ...
    showLoading();
    $.ajax({
      url: "<?php echo base_url('voices/add'); ?>",
      type: "POST",
      data: postData
    }).done(function(data) {
  ...
  var update = function(url) {
    ...
    showLoading();
    $.ajax({
      url: "<?php echo base_url('voices/update'); ?>",
      type: "POST",
      data: postData
    }).done(function(res) {
      var result = $.parseJSON(res);
  ...
  var deleteVoice = function() {
    ...
    Swal.fire({
      ...
      if (e.value) {
        showLoading();
        $.ajax({
          url: "<?php echo base_url('voices/delete'); ?>",
          type: "POST",
          data: postData
        }).done(function(data) {
          var result = $.parseJSON(data);
...
<script type="text/javascript">
  $(document).ready(function() {
    $("#menu-voices").addClass('mm-active');
