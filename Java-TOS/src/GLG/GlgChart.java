...
//////////////////////////////////////////////////////////////////////////
public class GlgChart extends GlgJLWBean implements ActionListener {
    ...
    // Parent container.
    GlgRTChartView ParentContainer;
    ...
    */
    ...
    int UpdateInterval = 70;            // Update interval in msec.
    ...
    */
    ...
    // Constructor.
    /////////////////////////////////////////////////////////////////////
    public GlgChart(GlgRTChartView parent) {
    ...
    public void HCallback(GlgObject viewport) {
        ...
        boolean update = Chart.SetDResource("NumPlots", 10);
        ...
        for (i = 0; i < NumPlots; ++i) {
            ...
            switch (i) {
                ...
                case 8:
                    ...
                    break;
                case 9:
                    Plots[i].SetGResource("EdgeColor", 0,0,255); /// blue // HP2
                    Plots[i].SetDResource("LineType", 0);
                    Plots[i].SetDResource("LineWidth", PreferenceManager.getLineWidth());
                    Plots[i].SetSResource("Annotation","HP2");
                    Plots[i].SetSResource("PlotAnnotation","HP2");

                    Plots[i].SetDResource("YLow", 0);
                    Plots[i].SetDResource("YHigh", 3);
                    break;
            }
    ...
    ///////////////////////////////////////////////////////////////////////
    public void setChartScale(float newScale) {
        if (!IsReady)
            return;

        if (chartScale == newScale || newScale <= 0)
            return;

        GlgObject axis_array = Chart.GetResourceObject("YAxisGroup");
        for (int i = 0; i < NumYAxes; ++i) {
            GlgObject axis = (GlgObject) axis_array.GetElement(i);
            axis.SetDResource("Low", 0);
            axis.SetDResource("High", newScale);
        }

        for (int i = 0; i < NumPlots; ++i) {
            Plots[i].SetDResource("YLow", 0);
            Plots[i].SetDResource("YHigh", newScale);
        }
    ...
    public void changeHighWearLimitLinesStyle(boolean isSolid) {
        ...
        if (isSolid) {
            Plots[1].SetDResource("LineType", 0);
            Plots[2].SetDResource("LineType", 0);
        } else {
            Plots[1].SetDResource("LineType", 6);
            Plots[2].SetDResource("LineType", 6);
        }
    }
    ...
    HSTFileSaveManager hstSaveManager;

    public void setFileSaveManager(HSTFileSaveManager fileSaveManager) {
        this.hstSaveManager = fileSaveManager;
    }
    ...
    public void UpdateChart() {
      ...
      */
        ...
        for (int i = 0; i < NumPlots; ++i) {
            ...
            */
            if (DataFeed.GetPlotPoint(i, data_point)) {
                PushPlotPoint(Plots[i], data_point);
                //System.out.println("357 glgchart.java");
        ...
        if (currChartEnd == 0) {
            currChartEnd = currTime + 60;
            Chart.SetDResource("XAxis/EndValue", currChartEnd);
        } else if(currTime >= currChartEnd) {
            currChartEnd = currTime + 30;
            Chart.SetDResource("XAxis/EndValue", currChartEnd);
        }

        if (hstSaveManager != null) {
            try {
                hstSaveManager.writeData(plotValues);
            } catch (Exception e) {
                e.printStackTrace();
            }
        }
...
//    }
    ...
    void error(String string, boolean quit) {
        System.out.println(string + "ERrror");
        if (quit)
            System.exit(0);
    }
